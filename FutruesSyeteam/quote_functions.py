...
def GetHistory(g_QuoteZMQ, g_QuoteSession , symbol, type, startTime, endTime):
    ...
    Sub_result = g_QuoteZMQ.SubHistory(g_QuoteSession, symbol, type, startTime, endTime)
    print("訂閱結果:",Sub_result)
    timeout = 100    
    ...
    dfAll = pd.read_json(StringIO(json.dumps(dataAll)))
    return dfAll

#行情消息接收
def quote_sub_th(obj,sub_port,stop,filter = "" ):
